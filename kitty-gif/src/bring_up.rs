...
fn first_and_last_four(s: &str) -> String {
    let chars: Vec<char> = s.chars().collect();
    let len = chars.len();

    if len <= 8 {
        return s.to_string(); // return full string if it's short
    }
...
pub fn init_window() {
    ...
    app.global::<UpdateFwCallback>().on_get_version(|| {
        ...
        let ota_info = ota.get_new_fw_info().unwrap();
        let version = ota_info.data.version.clone();
    ...
    });
    {
        ...
        app.set_deviceID(SharedString::from(first_and_last_four(DEVICE_ID)));
    }
